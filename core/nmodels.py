...
class Model:
    ...
    def get_move_apporximation(self, board, uci_move, priority=None):
        for move in board.legal_moves:
            if str(uci_move) == str(move):
                return str(uci_move)

        print(f"Made not valid move {uci_move}.")

        best_app_move = self.__find_closest_move(board, uci_move)

        return str(best_app_move)

    def __find_closest_move(self, board, uci_move):
        from_pos = self.__find_closest_pos([str(move)[:2] for move in board.legal_moves], uci_move.from_pos)
        to_pos = self.__find_closest_pos([str(move)[2:] for move in board.legal_moves if str(move)[:2] == from_pos], uci_move.to_pos)
        return UCIMove(from_pos + to_pos)

    def __find_closest_pos(self, legal_move_pos, pos):
        x1, y1 = bp.board_pos_to_num_pos(pos)

        smallest_dist = None
        best_x = None
        best_y = None

        for p in legal_move_pos:
            x2, y2 = bp.board_pos_to_num_pos(p)
            cur_dist = bp.dist2d(x1, y1, x2, y2)
            if not smallest_dist or smallest_dist > cur_dist:
                smallest_dist = cur_dist
                best_x = x2
                best_y = y2

        return bp.num_pos_to_board_pos([best_x, best_y])
...
class ConvNetModel(Model):
    ...
    def predict(self, board):
        ...
        uci_move = UCIMove(self.convert_move(model_move))
        uci_move = self.get_move_apporximation(board, uci_move)
        print(uci_move)
        return uci_move
...
# NModels.get_model().model.load_weights("../models/model_1_2048.h5")
# uci_move = NModels.get_model().predict(board)
# print(uci_move)

def get_conv_model(conv_depth, unit_size):
    model = keras.models.Sequential()
    model.add(layers.Conv2D(unit_size, (4, 4), input_shape=(8, 8, 1), activation="relu"))
...
# model1 = get_conv_model(1, 2048)
# model1.save("../models/my_model")
